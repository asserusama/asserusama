...
| `jbmono-head.woff2` | 600 | unused — see below |
| `jbmono-400.woff2` | 400 | basic latin, for the stat graphics |
| `jbmono-600.woff2` | 600 | basic latin, for the stat graphics |

`jbmono-head.woff2` covered only the letters in the original five section
headings. Adding a heading with an unseen glyph — the `x` in *experience*, the
`v` in *volunteer* — dropped that one character to the viewer's default
monospace, mismatched mid-word. The headings now use `jbmono-600.woff2`, which
is basic latin and already loaded for the stat graphics; the cost is about 3 KB
of extra base64 per heading. Kept on disk in case a re-subset is ever wanted.
