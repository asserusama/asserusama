...
Env:
  ...
  GH_LOGIN      user to summarise (default: asserusama)
...
def font_head():
    """Basic latin at 600 — so a heading can be any word.

    Was a 1.4 KB subset of just the original five headings' letters; adding a
    word with an unseen glyph silently dropped it to the viewer's default mono
    mid-word. The full face is 4.5 KB and already in the repo.
    """
...
def main():
    ...
    login = os.environ.get("GH_LOGIN", "asserusama")
    ...
    for word in ("about", "experience", "projects", "stack", "volunteer",
                 "stats"):
